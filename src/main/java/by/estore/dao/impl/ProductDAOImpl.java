...
public class ProductDAOImpl implements ProductDAO {
    ...
    private final String GET_ALL_PRODUCTS_QUERY = "SELECT p.`id`, p.`name` AS `product_name`, p.`description`, p.`image`, p.`category_id`, c.`name` AS `category_name` FROM `products` AS p JOIN `categories` AS c ON p.`category_id` = c.`id`";
    private final String GET_PRODUCTS_BY_CATEGORY_QUERY = "SELECT p.`id`, p.`name` AS `product_name`, p.`description`, p.`image`, p.`category_id` FROM `products` AS p JOIN `categories` AS c ON p.`category_id` = c.`id` WHERE c.`name` = ?";
    private final String GET_PRODUCTS_BY_CATEGORY_LIMIT_OFFSET_QUERY = "SELECT p.`id`, p.`name` AS `product_name`, p.`description`, p.`image`, p.`category_id` FROM `products` AS p JOIN `categories` AS c ON p.`category_id` = c.`id` WHERE c.`name` = ? LIMIT ? OFFSET ?";
    private final String GET_PRODUCT_BY_ID = "SELECT p.`name` AS `product_name`, p.`description`, p.`image`, p.`category_id`, c.`name` AS `category_name` FROM `products` AS p JOIN `categories` AS c ON p.`category_id` = c.`id` WHERE p.`id` = ?";
